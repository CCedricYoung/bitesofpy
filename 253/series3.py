...
def complex_series_maths(
    ...
    if function == "add":
        return ser_01.add(ser_02)
    if function == "sub":
        return ser_01.sub(ser_02)
    if function == "mul":
        return ser_01.mul(ser_02)
    if function == "div":
        return ser_01.div(ser_02)
...
def custom_series_function(ser: pd.Series, within: int) -> pd.core.series.Series:
    ...
    """
    # see https://en.wikipedia.org/wiki/Quartile#Computer_Software_for_Quartiles
    q = ser.describe().loc[["min", "25%", "50%", "75%", "max", "mean"]]

    def in_quartiles(num: int) -> bool:
        for x in q:
            if x - within <= num <= x + within:
                return True

        return False

    mask = ser.map(in_quartiles)  # create pandas mask series using function

    return ser[mask]  # filter pandas series by mask series
