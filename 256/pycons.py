...
def get_pycon_events(data=_get_pycon_data()) -> List[PyCon]:
    ...
    pycons = [
        PyCon(
            ...
            country=x['location']['address']['addressCountry'],
            start_date=parse(x['startDate']),
            end_date=parse(x['endDate']),
            url=x['url'],
        )
        for x in events
    ]
    return sorted(pycons, key=itemgetter(1))
...
def filter_pycons(pycons: List[PyCon],
    ...
    result = [
        x
        for x in pycons
        if x.start_date.year <= year <= x.end_date.year
        and get_continent(x.country) == continent
    ]
