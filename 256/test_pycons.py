...
from pycons import get_pycon_events, filter_pycons, get_continent
...
def test_get_pycon_events_dates(pycon_events):
    assert all(
        isinstance(event.start_date, datetime.datetime) for event in pycon_events
    )
    assert all(isinstance(event.end_date, datetime.datetime) for event in pycon_events)
...
def test_filter_pycons_dates(filtered_pycons):
    assert all(
        isinstance(event.start_date, datetime.datetime) for event in filtered_pycons
    )
    assert all(
        isinstance(event.end_date, datetime.datetime) for event in filtered_pycons
    )
