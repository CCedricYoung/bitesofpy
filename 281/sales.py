...
def process_data(url: str) -> pd.DataFrame:
    ...
    """
    data = get_data(url)
    csv_data = StringIO(b64decode(data["content"]).decode("utf-8"))
...
def summary_report(df: pd.DataFrame, stats: Union[List[str], None] = STATS) -> None:
    ...
    df["year"] = df.apply(lambda x: datetime.fromisoformat(x["month"]).year, axis=1)
    print(df[["year", "sales"]].groupby(["year"])["sales"].aggregate(stats))
...
def yearly_report(df: pd.DataFrame, year: int) -> None:
    ...
    """
    df["year"] = df.apply(lambda x: datetime.fromisoformat(x["month"]).year, axis=1)
    df["month"] = df.apply(lambda x: datetime.fromisoformat(x["month"]).month, axis=1)
    ...
    results = df[["month", "sales"]].to_string(index=False, justify="left")
