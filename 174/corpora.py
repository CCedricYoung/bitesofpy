...
class Corpora:
    ...
    def cleaned(self) -> str:
        ...
        * If a list of extra characters were given, remove those too
        ...
        """
        clean = self.txt.lower() + '\n' # off by 1 so add a char
        clean = clean.translate(str.maketrans('', '', string.punctuation))
        for x in sorted(self.extra, key=len, reverse=True):
            clean = clean.replace(x.lower(), ' ')
    ...
    def metrics(self) -> List[Tuple[str, int]]:
        ...
        * Use count to return the requested amount of the top words, defaults to 5
        ...
        more_cleaned = self.cleaned.replace('—', ' ')
        words = [x for x in more_cleaned.split() if x not in stopwords]
        return collections.Counter(words).most_common(self.count)
    ...
    def graph(self) -> None:
        ...
        * The tag is repeated the number of counts of the word
        ...
        for x in self.metrics:
            print(f'{x[0]:>10} {self.tag * x[1]}')


a = Corpora(GETTYSBURG)
print(a.cleaned)
print(len(a.cleaned))
# a.extra = ['an', 're']
print(a.metrics)
